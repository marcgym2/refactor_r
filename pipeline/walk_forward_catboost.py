"""
CatBoost walk-forward backtest using the same causal fold construction
and portfolio-return logic as the FFNN baseline.

Writes:

- forecasts/portfolio_walk_forward_catboost_backtest.csv
- forecasts/portfolio_walk_forward_catboost_summary.json
"""
...
def _train_model_for_fold(
    *,
    ...
    feature_names: list[str],
) -> tuple[CatBoostClassifier, dict[str, float], dict[str, float | int | None]]:
    ...
    test_processed = _apply_preprocessing(test_frame, feature_names=feature_names, medians=medians)

    x_train, y_train = _prepare_xy(train_processed, feature_names)
    x_test, y_test = _prepare_xy(test_processed, feature_names)
    ...
    best_score = None
    try:
        best_score = float(model.get_best_score()["validation"]["TotalF1:average=Macro"])
    except Exception:
        best_score = None
...
def run(
    *,
    ...
    if not feature_names:
        raise RuntimeError("No feature columns were produced for CatBoost walk-forward training.")
    ...
    for fold in folds:
        ...
        if fold.retrain or current_model is None or current_medians is None or current_fit_info is None:
            print(
                "[WalkForwardCatBoost] Retraining fold "
        ...
        records.append(
            {
                ...
                "BestIteration": current_fit_info["best_iteration"],
                "BestDevScore": current_fit_info["best_dev_score"],
    ...
    if not records:
        raise RuntimeError("CatBoost walk-forward run produced no portfolio records.")

    backtest = pd.DataFrame(records)
    backtest_path = os.path.join(FORECASTS_DIR, "portfolio_walk_forward_catboost_backtest.csv")
    backtest.to_csv(backtest_path, index=False)

    summary = _build_summary(backtest, cfg, started_at)
    summary_path = os.path.join(FORECASTS_DIR, "portfolio_walk_forward_catboost_summary.json")
    with open(summary_path, "w", encoding="utf-8") as handle:
        json.dump(summary, handle, indent=2)

    metrics = summary["models"]["catboost"]
    print(f"[WalkForwardCatBoost] Saved backtest -> {backtest_path}")
    print(f"[WalkForwardCatBoost] Saved summary -> {summary_path}")
    print(
        "[WalkForwardCatBoost] "
